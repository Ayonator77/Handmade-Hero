...
internal void draw_pixel(win32_offscreen_buffer *buffer, int x, int y, uint8 r,
                         uint8 g, uint8 b) {
...
internal void RenderWeirdGradient(win32_offscreen_buffer buffer, int x_offset,
                                  int y_offset) {
...
internal void Win32DispalyBufferInWindow(HDC device_context, int window_width,
                                         int window_height,
...
LRESULT CALLBACK Win32MainWindowCallback(HWND Window, UINT Message,
  ...
  case WM_SIZE: {
    ...
    Win32ResizeDIBSection(&global_back_buffer, dimension.width,
                          dimension.height);
  ...
  case WM_PAINT: {
    ...
    RECT client_rect;
    GetClientRect(Window, &client_rect);
    Win32DispalyBufferInWindow(device_context, dimension.width,
                               dimension.height, global_back_buffer, X, Y,
                               width, height);
...
int CALLBACK WinMain(HINSTANCE Instance, HINSTANCE PrevInstance, LPSTR CmdLine,
  ...
  // MessageBoxExW(NULL, L"This is a test", L"Handmade Hero", MB_OK |
  ...
  if (RegisterClassW(&window_class)) {
    ...
    HWND window = CreateWindowExW(
        0, window_class.lpszClassName, L"Handmade Hero",
    ...
    if (window) {
      ...
      while (running) {
        ...
        Win32DispalyBufferInWindow(device_context, dimension.width,
                                   dimension.height, global_back_buffer, 0, 0,
                                   window_wiidth, window_height);
